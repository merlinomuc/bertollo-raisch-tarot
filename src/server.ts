...
function createServer() {
  const server = new McpServer({ name: "bertollo-raisch-tarot", version: "1.3.9" });
  ...
  server.registerTool(
    ...
    async () => ({
      ...
      structuredContent: { app: "bertollo-raisch-tarot", version: "1.3.9", language: "de", provider: "Bertollo", storesUserData: false },
...
app.get("/health", (_req, res) => res.json({ ok: true, service: "bertollo-raisch-tarot", version: "1.3.9", aiConfigured: Boolean(process.env.OPENAI_API_KEY) }));
